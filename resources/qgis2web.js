...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
        extent: [-811295.046397, 7295255.968001, -801752.407498, 7300477.079118], maxZoom: 20, minZoom: 1
    })
});
...
map.getView().fit([-811295.046397, 7295255.968001, -801752.407498, 7300477.079118], map.getSize());
                    ...
                    for (var i=0; i<currentFeatureKeys.length; i++) {
                        if (currentFeatureKeys[i] != 'geometry') {
                            ...
                            if (layer.get('fieldImages')[currentFeatureKeys[i]] != "ExternalResource") {
                                ...
                                popupField += (currentFeature.get(currentFeatureKeys[i]) != null ? '<a href="images/' + currentFeature.get(currentFeatureKeys[i]).replace(/[\\\/:]/g, '_').trim() + '" /" target="blank"><img src="images/' + currentFeature.get(currentFeatureKeys[i]).replace(/[\\\/:]/g, '_').trim() + '" / width="400" height="400"></a></td>' : '');
                            }
                            popupText += '<tr>' + popupField + '</tr>';
                        }
                    }
